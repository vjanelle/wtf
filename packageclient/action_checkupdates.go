...
// CheckupdatesRequester performs a RPC request to package#checkupdates
type CheckupdatesRequester struct {
	r    *requester
	outc chan *CheckupdatesOutput
}

// CheckupdatesOutput is the output from the checkupdates action
type CheckupdatesOutput struct {
	details *ResultDetails
	reply   map[string]interface{}
}

// CheckupdatesResult is the result from a checkupdates action
type CheckupdatesResult struct {
	ddl        *agent.DDL
	stats      *rpcclient.Stats
	outputs    []*CheckupdatesOutput
	rpcreplies []*replyfmt.RPCReply
	mu         sync.Mutex
}

func (d *CheckupdatesResult) RenderResults(w io.Writer, format RenderFormat, displayMode DisplayMode, verbose bool, silent bool, colorize bool, log Log) error {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *CheckupdatesResult) Stats() Stats {
	return d.stats
}

// ResultDetails is the details about the request
func (d *CheckupdatesOutput) ResultDetails() *ResultDetails {
	return d.details
}

// HashMap is the raw output data
func (d *CheckupdatesOutput) HashMap() map[string]interface{} {
	return d.reply
}

// JSON is the JSON representation of the output data
func (d *CheckupdatesOutput) JSON() ([]byte, error) {
	return json.Marshal(d.reply)
}

// ParseCheckupdatesOutput parses the result value from the Checkupdates action into target
func (d *CheckupdatesOutput) ParseCheckupdatesOutput(target interface{}) error {
	j, err := d.JSON()
	if err != nil {
...
func (d *CheckupdatesRequester) Do(ctx context.Context) (*CheckupdatesResult, error) {
	dres := &CheckupdatesResult{ddl: d.r.client.ddl}
...
func (d *CheckupdatesResult) EachOutput(h func(r *CheckupdatesOutput)) {
...
func (d *CheckupdatesOutput) Exitcode() interface{} {
...
func (d *CheckupdatesOutput) OutdatedPackages() interface{} {
...
func (d *CheckupdatesOutput) Output() interface{} {
...
// PackageManager is the value of the package_manager output
//
// Description: The detected package manager
func (d *CheckupdatesOutput) PackageManager() interface{} {
	val, ok := d.reply["package_manager"]
	if !ok || val == nil {
		// we have to avoid returning nil.(interface{})
		return nil
	}
